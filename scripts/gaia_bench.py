...
VERIFY_HINT = (
    ...
    "For a numeric value you READ off a fetched page/file, RE-EXTRACT it "
    "deterministically from the SAVED source instead of reading it by eye:\n"
    f"    python {os.path.join(WS_ROOT, 'data', 'bench', 'tooltest', 'verify_candidate.py')} "
    "verify --answer '<your answer>' --source-file '<saved source>' "
    "--extract 'csv:col=N[,skip=M][,agg=sum|avg] | table:N | regex:PATTERN | label:TEXT[,agg=sum|avg] | num'\n"
