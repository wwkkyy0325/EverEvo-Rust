...
use oxigraph::model::{
    GraphName, Literal, NamedNode, NamedOrBlankNode, Quad, Term,
};
...
fn quad_named(s: NamedNode, p: NamedNode, o: Term) -> Quad {
    Quad::new(NamedOrBlankNode::NamedNode(s), p, o, GraphName::DefaultGraph)
}
...
impl KnowledgeGraph {
    pub fn open(dir: impl Into<PathBuf>) -> Result<Self, EverEvoError> {
        ...
        let store = Store::new()
            .map_err(|e| EverEvoError::KnowledgeGraph(format!("Create store: {e}")))?;
        let mut kg = Self {
    ...
    pub fn merge_entities(&mut self, canonical_id: &str, merged_id: &str) {
        ...
        if let Some(canonical) = self.entities.get_mut(canonical_id) {
            for (k, v) in &merged.properties {
                canonical.properties.entry(k.clone()).or_insert_with(|| v.clone());
            }
            for src in &merged.sources {
                if !canonical.sources.iter().any(|s| s.message_id == src.message_id) {
        ...
        if let Ok(QueryResults::Solutions(solutions)) = self.store.query(&sparql) {
            for solution in solutions {
                if let Ok(sol) = solution {
                    let id = sol.get("id").map(|t| term_value(t)).unwrap_or_default();
                    let label = sol.get("label").map(|t| term_value(t)).unwrap_or_default();
                    let etype = sol.get("type").map(|t| term_value(t)).unwrap_or_default();
                    if id.is_empty() {
                        continue;
                    }
                    self.entities.insert(
                        ...
                        Entity {
                            ...
                            updated_at: sol
                                ...
                                .map(|dt| dt.with_timezone(&chrono::Utc))
                                .unwrap_or_else(chrono::Utc::now),
                            merged_into: sol.get("merged").map(|t| term_value(t)),
                        },
                    );
                }
            }
    ...
    pub fn query_sparql(
        &self,
        sparql: &str,
    ) -> Result<Vec<HashMap<String, String>>, EverEvoError> {
        let results = self
            .store
            .query(sparql)
    ...
    pub fn save(&self) -> Result<(), EverEvoError> {
        ...
        for entity in self.entities.values() {
            ...
            self.store
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert type: {e}"))
                })?;
            self.store
                .insert(&quad_named(
                    s.clone(),
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert label: {e}"))
                })?;
            self.store
                .insert(&quad_named(
                    s.clone(),
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert entityType: {e}"))
                })?;
            self.store
                .insert(&quad_named(
                    s.clone(),
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert createdAt: {e}"))
                })?;
            self.store
                .insert(&quad_named(
                    s.clone(),
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert updatedAt: {e}"))
                })?;
            let props_json =
            ...
            self.store
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert properties: {e}"))
                })?;
            let sources_json =
            ...
            self.store
                ...
                .map_err(|e| {
                    EverEvoError::KnowledgeGraph(format!("Insert sources: {e}"))
                })?;
            if let Some(ref m) = entity.merged_into {
                self.store
                    ...
                    .map_err(|e| {
                        EverEvoError::KnowledgeGraph(format!("Insert mergedInto: {e}"))
                    })?;
            }
        }
        self.store
