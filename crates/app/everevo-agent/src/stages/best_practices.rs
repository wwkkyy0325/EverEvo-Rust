//! In-process BestPractices ContextStage for the context pipeline.
//!
//! Complemented by MCP stage plugin `plugin-stage-best-practices` which exposes MCP
//! tools the agent can call explicitly. This in-process version implements the
//! ContextStage trait for automatic injection into the LLM context — a capability
//! MCP plugins cannot provide (MCP has no ContextStage equivalent).
