//! In-process DomainKnowledge ContextStage for the context pipeline.
//!
//! Complemented by MCP stage plugin `plugin-stages-domain` which exposes MCP tools.
//! This in-process version implements ContextStage for automatic domain document
//! injection into the LLM context — a capability MCP plugins cannot provide.
