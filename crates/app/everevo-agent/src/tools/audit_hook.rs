//! In-process AuditHook — automatic ToolHook that logs every tool call.
//!
//! Complemented by MCP hook plugin `plugin-hooks-audit` which exposes the `audit_log`
//! tool for explicit agent calls. This in-process version implements the ToolHook trait
//! for AUTOMATIC execution on every tool call — a capability MCP plugins cannot provide
//! (MCP has no ToolHook equivalent; its tools must be called explicitly).
