//! In-process ReviewGateHook — automatic PRE-ACT tool call validation.
//!
//! Complemented by MCP hook plugin `plugin-hooks-review_gate` which exposes the
//! `review_pre_execute` tool for explicit agent calls. This in-process version
//! implements ToolHook::pre_execute() for AUTOMATIC pre-execution gating on every
//! tool call — a capability MCP plugins cannot provide.
...
impl ToolHook for ReviewGateHook {
    /// Reset redundancy tracking at the START of each turn.
    /// Without this, `last_call` would persist across turns AND sessions,
    /// causing false positives — any legitimate repeated call (read same file,
    /// run same workflow) would be misidentified as a "fixation loop".
