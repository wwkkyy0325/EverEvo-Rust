...
) -> everevo_agent::AgentLoop {
    agent = agent
        .with_proactivity(std::sync::Arc::clone(proactivity))
        // Drive the agent loop's own context ceiling from the per-model budget
        // (4 chars/token, matching driver.rs `max_context_chars / 4`).
        .with_context_budget(context_window_tokens * 4)
