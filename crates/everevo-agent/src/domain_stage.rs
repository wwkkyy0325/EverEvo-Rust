...
use everevo_domain::DomainManager;
    ...
    fn find_relevant(
        ...
        for (id, domain) in &mgr.registry.domains {
                ...
                .contains(&query_lower)
            {
                ...
                (query_words.iter().filter(|w| domain.description.to_lowercase().contains(*w)).count() as f32)
...
impl ContextStage for DomainKnowledgeStage {
    ...
    fn build(&self, ctx: &ContextBuildContext) -> Option<ContextFragment> {
        ...
        lines.push("The following knowledge-base documents are relevant to the current conversation:\n".into());
