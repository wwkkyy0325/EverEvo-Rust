...
impl WikiGenerator {
    ...
    pub fn new(wiki_dir: impl Into<PathBuf>) -> Result<Self, EverEvoError> {
        ...
        std::fs::create_dir_all(&wiki_dir).map_err(|e| {
            EverEvoError::Internal(format!("Create wiki dir: {e}"))
        })?;
        Ok(Self { wiki_dir, llm: None })
    }
    ...
    async fn update_page(
        ...
        std::fs::write(&path, &content).map_err(|e| {
            EverEvoError::Internal(format!("Write wiki page {page_path}: {e}"))
        })?;
    ...
    async fn create_page(
        ...
        std::fs::write(&path, &content).map_err(|e| {
            EverEvoError::Internal(format!("Create wiki page {filename}: {e}"))
        })?;
    ...
    async fn generate_page_with_llm(
        ...
        Ok(response.content.unwrap_or_else(|| self.template_page(fact, related)))
    }
    ...
    fn template_page(&self, fact: &MemoryFact, related: &[MemoryFact]) -> String {
        ...
        if related.is_empty() {
            ...
            for r in related {
                content.push_str(&format!("- [{name}](../facts/{name}.md) — {desc}\n", name = r.name, desc = r.description));
            }
            content.push('\n');
        }

        content.push_str("## References\n\n");
        content.push_str(&format!("- [{name}](../facts/{name}.md) — source fact\n", name = fact.name));
    ...
    fn collect_files(&self, dir: &Path, prefix: &str, pages: &mut Vec<WikiPage>) -> Result<(), EverEvoError> {
        if !dir.exists() { return Ok(()); }
        for entry in std::fs::read_dir(dir).map_err(|e| EverEvoError::Internal(format!("Read dir: {e}")))? {
            ...
            if path.is_dir() {
                ...
                let title = content.lines().find(|l| l.starts_with("# ")).map(|l| l[2..].to_string()).unwrap_or_else(|| name.clone());
                let page_path = if prefix.is_empty() { name } else { format!("{prefix}/{name}") };
                pages.push(WikiPage { path: page_path, title, content, updated_at: chrono::Utc::now(), references: vec![] });
            }
        }
        Ok(())
    }
    ...
    fn append_changelog_entry(&self, facts: &[MemoryFact], action: &str) -> Result<(), EverEvoError> {
        let path = self.wiki_dir.join("changelog.md");
        let today = chrono::Utc::now().format("%Y-%m-%d").to_string();
        let mut entry = format!("\n## {today}\n\n");
        for f in facts {
            entry.push_str(&format!("- **{action}**: [{name}](../facts/{name}.md) — {desc}\n", action = action, name = f.name, desc = f.description));
        }
        let mut content = if path.exists() { std::fs::read_to_string(&path).unwrap_or_default() } else { String::from("# Changelog\n\nAuto-generated by EverEvo Memory Pipeline.\n") };
        if let Some(pos) = content.find('\n') { content.insert_str(pos + 1, &entry); } else { content.push_str(&entry); }
        std::fs::write(&path, &content).map_err(|e| EverEvoError::Internal(format!("Write changelog: {e}")))?;
        Ok(())
    }

    pub fn wiki_dir(&self) -> &Path { &self.wiki_dir }
}
...
mod tests {
    ...
    fn make_fact(name: &str, desc: &str, content: &str) -> MemoryFact {
        MemoryFact {
            name: name.into(), description: desc.into(), content: content.into(),
            fact_type: FactType::Project,
            created_at: chrono::Utc::now(), updated_at: chrono::Utc::now(),
            projection: ProjectionMetadata::new("test", "none", vec![], 1.0), links: vec![],
        }
    }
