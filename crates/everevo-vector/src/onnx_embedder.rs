...
pub fn configure_ort_dylib(data_dir: &Path) {
    ...
    if !dll.exists() {
        tracing::warn!(
            "onnxruntime.dll not found at {} — falling back to system",
            dll.display()
        );
        return;
    }
...
#[cfg(not(feature = "onnx"))]
impl OnnxEmbedder {
    pub fn new(_key: &str, _dir: impl Into<PathBuf>) -> Result<Self, EverEvoError> {
        Ok(Self)
    }
...
pub fn check_onnx_model(key: &str, _dir: &std::path::Path) -> Option<OnnxCheckResult> {
    Some(OnnxCheckResult {
        model_key: key.into(),
        loaded: false,
        smoke_passed: false,
        error: Some("onnx feature off".into()),
    })
}
...
fn is_ort_compatible(data_dir: &std::path::Path) -> bool {
    *ORT_VERSION_OK.get_or_init(|| {
        ...
        match vf.and_then(|p| std::fs::read_to_string(p).ok()) {
            ...
            None => {
                tracing::warn!("Cannot read ORT version — ONNX disabled");
                false
            }
        }
    })
...
pub struct OnnxEmbedder {
    dim: usize,
    model_key: String,
    inner: Option<Mutex<fastembed::TextEmbedding>>,
}
    ...
    pub fn new(model_key: &str, models_dir: impl Into<PathBuf>) -> Result<Self, EverEvoError> {
        ...
        let inner = if is_ort_compatible(data_dir) {
            match load_fastembed(model_key, &model_dir) {
                Ok(e) => {
                    tracing::info!(key=%model_key, "ONNX model loaded");
                    Some(Mutex::new(e))
                }
                Err(e) => {
                    tracing::warn!(key=%model_key, err=%e, "fastembed init failed");
                    None
                }
            }
        } else {
...
fn load_fastembed(
    ...
    let onnx =
        std::fs::read(model_dir.join("model_quantized.onnx")).map_err(|e| format!("onnx: {e}"))?;
    ...
    TextEmbedding::try_new_from_user_defined(m, fastembed::InitOptionsUserDefined::new())
        .map_err(|e| format!("{e}"))
}
...
impl EmbeddingModel for OnnxEmbedder {
    fn encode(&self, text: &str) -> Result<Vec<f32>, EverEvoError> {
        let Some(ref i) = self.inner else {
            return Ok(vec![0.0_f32; self.dim]);
        };
        let mut e = i
            .lock()
            .map_err(|e| EverEvoError::Internal(format!("lock: {e}")))?;
        let r = e
            .embed(vec![text], None)
            .map_err(|e| EverEvoError::Vector(format!("encode: {e}")))?;
        Ok(r.into_iter()
            .next()
...
pub fn check_onnx_model(model_key: &str, models_dir: &std::path::Path) -> Option<OnnxCheckResult> {
    ...
    if !is_ort_compatible(data_dir) {
        return Some(OnnxCheckResult {
            model_key: model_key.into(),
            loaded: false,
            smoke_passed: false,
            error: Some("ORT too old".into()),
        });
    }
    match OnnxEmbedder::new(model_key, models_dir) {
        Ok(e) => {
            ...
            Some(OnnxCheckResult {
                ...
                loaded: e.is_loaded(),
                smoke_passed: ok,
                error: if ok { None } else { Some("zero vector".into()) },
            })
        }
        Err(e) => Some(OnnxCheckResult {
            model_key: model_key.into(),
            loaded: false,
            smoke_passed: false,
