...
impl Tool for BootstrapWriteFile {
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "File path to write (kernel paths blocked)"},
    ...
    async fn execute(
        ...
        let content = params["content"].as_str().ok_or_else(|| EverEvoError::Tool {
            tool: "write_file".into(),
            message: "Missing 'content' parameter".into(),
        })?;

        // ── Kernel self-protection chokepoint ─────────────────────────
        if protection::is_kernel_protected(path) {
            return Ok(ToolOutput {
                content: format!(
                    "BLOCKED: '{path}' is in a kernel-protected area.\n\
        ...
        match std::fs::write(path, content) {
            Ok(()) => Ok(ToolOutput::text(format!("Wrote {} bytes to '{path}'", content.len()))),
            Err(e) => Ok(ToolOutput {
