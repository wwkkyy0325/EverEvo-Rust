...
async fn list_models(State(state): State<Arc<AppState>>) -> Json<ModelsResponse> {
    let reg = state.model_registry.read().unwrap_or_else(|e| e.into_inner());
    let active = reg.active();
    Json(ModelsResponse {
        models: reg.list().iter().map(|m| ModelInfo {
            name: m.name.clone(),
...
#[derive(Deserialize)]
struct ActivateRequest { model: String }
...
async fn activate_model(
    ...
    Ok(Json(ModelInfo { name: meta.name, display_name: meta.display_name, dim: meta.dim, active: true }))
}

#[derive(Serialize)]
struct ReindexResponse { collection: String, processed: usize, duration_ms: u64 }
...
async fn reindex_collection(
    ...
    let collection = req.get("collection").and_then(|v| v.as_str()).unwrap_or("memory");
    let rag = state.rag_pipeline.as_ref().ok_or_else(|| {
        (StatusCode::SERVICE_UNAVAILABLE, "RAG pipeline not initialized".into())
    })?;
...
pub fn routes() -> axum::Router<Arc<AppState>> {
    axum::Router::new()
        ...
        .route("/api/vector/reindex", axum::routing::post(reindex_collection))
}
