...
use everevo_vector::{EmbeddingModel, MemoryChunk, RawChunk, VectorStore, ChunkType as VectorChunkType};
...
impl DomainManager {
    ...
    pub fn list_documents(&self, domain_id: &str) -> Result<Vec<DocumentMeta>, EverEvoError> {
        ...
        {
            let entry =
                entry.map_err(|e| EverEvoError::Internal(format!("Entry: {e}")))?;
    ...
    pub async fn process_global_inbox(&mut self) -> Result<InboxResult, EverEvoError> {
        ...
        for (filename, content) in &files {
            ...
            let doc_vector: Vec<f32> = if let Some(ref emb) = self.embedder {
                emb.encode(&text).unwrap_or_else(|_| vec![0.1_f32; self.registry.embedding_dim])
            } else {
            ...
            if let (Some(ref emb), Some(ref store)) = (&self.embedder, &self.vector_store) {
                let raw_chunks: Vec<RawChunk> = chunks.iter().map(|c| {
                    RawChunk {
                        id: uuid::Uuid::new_v4(),
                        content: c.clone(),
                        source_pointers: vec![],
                ...
                if let Ok(vectors) = emb.encode_batch(&raw_chunks.iter().map(|c| c.content.clone()).collect::<Vec<_>>()) {
                    let mchunks: Vec<MemoryChunk> = raw_chunks.into_iter().zip(vectors).map(|(raw, vector)| {
                        MemoryChunk {
                            id: raw.id, content: raw.content, vector,
                            source_pointers: raw.source_pointers, projection: raw.projection,
                            chunk_type: raw.chunk_type, created_at: chrono::Utc::now(), retrieval_count: 0,
                        }
                    }).collect();
