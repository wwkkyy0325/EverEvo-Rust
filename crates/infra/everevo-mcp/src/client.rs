//! MCP client — supports stdio and Streamable HTTP transports.
//!
//! Claude Code alignment:
//! - stdio: spawns a child process, JSON-RPC over stdin/stdout
//! - HTTP: Streamable HTTP (2025-03-26 spec), POST to /mcp endpoint
//!
//! NOTE: HTTP+SSE transport was **deprecated** in MCP 2025-03-26 in favor of
//! Streamable HTTP. The SSE response parsing below handles legacy servers that
//! still use SSE framing for responses. New code should NOT implement SSE
//! transport (GET /sse → session endpoint → POST) — use Streamable HTTP.
...
impl McpClient {
    ...
    pub async fn connect_stdio(
        ...
        // Windows: prevent console windows, isolate in new process group
        // CREATE_NO_WINDOW=0x08000000 suppresses console window
        // CREATE_NEW_PROCESS_GROUP=0x00000200 prevents signal propagation from parent
        #[cfg(windows)]
        cmd.creation_flags(0x08000000u32 | 0x00000200u32);
        for (k, v) in env {
            cmd.env(k, v);
        }
        let mut child = cmd
    ...
    /// Wrapped with a per-request timeout (request_timeout_secs, default 120s)
    /// to prevent hung MCP plugins from blocking the agent loop indefinitely.
    pub async fn call_tool(
        ...
        let timeout_dur = std::time::Duration::from_secs(self.request_timeout_secs);
        ...
        let result = match tokio::time::timeout(timeout_dur, send_fut).await {
            Ok(r) => r?,
            Err(_elapsed) => {
                return Err(format!(
                    "MCP tool '{}' timed out after {}s",
                    name, self.request_timeout_secs
                ));
            }
        };
    ...
    /// Graceful shutdown per MCP lifecycle: send `shutdown` request,
    /// wait for acknowledgment, then send `exit` notification (stdio only).
    /// For HTTP transport, this is a no-op (stateless connection).
...
impl Drop for McpClient {
    fn drop(&mut self) {
        // Staged termination (best-effort, follows Claude Code pattern):
        // 1. Close stdin → server detects EOF → graceful shutdown
        // 2. Child is dropped → process group terminates
        // HTTP transports are stateless — no cleanup needed.
        if matches!(self.transport, Transport::Stdio { .. }) {
            tracing::debug!("McpClient dropped — staged termination: close stdin → drop child");
        }
    }
}
