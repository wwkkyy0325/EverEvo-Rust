...
use crate::trace::should_sample;
use crate::writer::run_writer;
use everevo_core::EverEvoError;

// Re-exports (also serve as internal `use` for items used by Telemetry impl)
pub use crate::config::TelemetryConfig;
pub use crate::records::{AgentTurnRecord, RetrievalRecord};
pub use crate::trace::{SpanGuard, Trace};
        ...
        let handle = std::thread::Builder::new()
            ...
            .map_err(|e| {
                EverEvoError::Internal(format!(
                    "failed to spawn telemetry writer thread: {e}"
                ))
            })?;
...
mod tests {
    ...
    fn span_drop_writes_to_db() {
        ...
        rt.block_on(async {
            let pool = crate::writer::create_pool(&db_path).await.unwrap();

            let row: (String, String, String, i64) = sqlx::query_as(
                "SELECT trace_id, name, status, duration_ms FROM telemetry_spans",
            )
            .fetch_one(&pool)
            .await
            .unwrap();
